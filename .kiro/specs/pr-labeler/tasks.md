...
- [x] 1.1 新規ライブラリの追加とプロジェクト設定の更新
  - eslintcc（循環的複雑度計算）とjs-yaml（YAML設定パース）を依存関係に追加
...
- [ ] 1.3 複雑度ライブラリのPoCと互換性検証
  - eslintccのPoC実装（.ts/.tsxのAST解析、関数単位の複雑度取得）
...
- [ ] 3. 循環的複雑度分析機能の実装
- [ ] 3.1 eslintccとの統合と基本的な複雑度計算
  - eslintccライブラリを使用してTypeScript/JavaScriptファイルの複雑度を計算
...
- [ ] 9.4 複雑度分析失敗フローのテスト
  - eslintccエラー → 警告ログ → 複雑度ラベルなしで継続のフロー
  - eslintccモック（throw Error）
