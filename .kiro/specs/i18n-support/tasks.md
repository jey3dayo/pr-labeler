...
- [ ] 1. i18n基盤の構築
- [ ] 1.1 i18nextライブラリの統合と初期化システムの構築
...
- [ ] 2. GitHub Actions Summaryの多言語化
- [ ] 2.1 数値とファイルサイズのフォーマット機能の実装
...
- [ ] 7. 統合テストとスナップショット整備
- [ ] 7.1 言語別スナップショットテストの実装
  - PRコメント出力のスナップショット（英語・日本語）
  - エラーメッセージのスナップショット（英語・日本語）
  - ログメッセージのスナップショット（英語・日本語）
  - _Requirements: 2.2, 2.3, 2.4_

- [ ] 7.2 フォールバック動作のテスト実装
  - 翻訳キー欠落時の英語フォールバックテスト
  - 不正言語コード時のフォールバックテスト
  - フォールバック監査ログの動作確認
  - _Requirements: 1.4, 1.5, 1.6_

- [ ] 7.3 エンドツーエンド統合テストの実装
  - 環境変数指定時の全出力多言語化テスト
  - 設定ファイル指定時の全出力多言語化テスト
  - 言語未指定時の英語出力テスト
  - 既存英語環境での回帰テスト（後方互換性）
