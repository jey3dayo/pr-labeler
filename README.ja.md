...
- `auto/large-files` - 個別ファイルが大きすぎる（`file_size_limit_enabled: "false"` で無効化）
...
- `auto/excessive-changes` - 追加行数合計が閾値を超過（`pr_additions_limit_enabled: "false"` で無効化）
